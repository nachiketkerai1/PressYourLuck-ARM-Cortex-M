...
joystick_position_t joystick_get_pos(void)
{
    ...
    if (y > JOYSTICK_THRESH_Y_UP_2P475V){
        // Joystick in the top half
        if (x > JOYSTICK_THRESH_X_LEFT_2P475V){
            return JOYSTICK_POS_UPPER_LEFT;
        } else if (x < JOYSTICK_THRESH_X_RIGHT_0P825V){
            return JOYSTICK_POS_UPPER_RIGHT;
        } else {
            return JOYSTICK_POS_UP;
        }
    } else if (y < JOYSTICK_THRESH_Y_DOWN_0P825V){
        if (x > JOYSTICK_THRESH_X_LEFT_2P475V){
...
void joystick_print_pos(joystick_position_t position)
{
    if (position == JOYSTICK_POS_DOWN){
        printf("Down");
    } else if (position == JOYSTICK_POS_CENTER){
        printf("Center");

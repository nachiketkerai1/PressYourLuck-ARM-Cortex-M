...
bool circular_buffer_add(Circular_Buffer *buffer, char c)
{
  ...
  uint16_t index = buffer->produce_count % buffer->max_size;
  buffer->data[index] = c;
  //printf("Here adding: %d\n",(uint8_t)buffer->data[index] )
...
{
  uint16_t numChars = buffer->produce_count - buffer->consume_count;
  /* ADD CODE */
  return sizeof(char) * numChars;
}
...
char circular_buffer_remove(Circular_Buffer *buffer)
{
  ...
  uint16_t index = buffer->consume_count % buffer->max_size;
